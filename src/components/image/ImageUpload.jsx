...
const ImageUpload = ({ name, className = "", progress = 0, image, handleDeleteImage = () => {}, ...rest }) => {
  return (
    <label
      className={`${className} cursor-pointer flex items-center justify-center group bg-gray-100 border w-full min-h-[200px] rounded-lg border-dashed relative overflow-hidden`}
    >
      <input type="file" name={name} className="hidden-input" onChange={() => {}} {...rest} />
      {progress !== 0 && !image && (
        <div className="absolute z-10 w-16 h-16 border-8 border-green-500 rounded-full border-t-transparent animate-spin"></div>
      )}

      {!image && progress === 0 && (
        <div className="flex flex-col items-center text-center pointer-events-none">
          <img src="/img-upload.png" alt="upload-img" className="max-w-[80px] mb-5" />
          <p className="font-semibold">Choose phote</p>
        </div>
      )}
